...
class BusquedaController extends Controller
{
    public function buscar(Request $request)
    {
        ...
        if ($soloLicencias) {
            // 🔥 SOLO MOSTRAR LICENCIAS CON CONTADOR DE EQUIPOS - LIMIT 100
            $licencias = DB::select("
                SELECT 
                    'licencia' AS tipo,
                    l.idLicencia,
        ...
        // 1. BUSCAR ARTÍCULOS CON FILTROS (INCLUYE GRUPOS) - LIMIT 100
        $articulos = DB::select("
            SELECT 
                'articulo' AS tipo,
                a.idArticulo,
        ...
        // 2. BUSCAR LICENCIAS - SOLO SI NO HAY FILTROS DE TIPO/ÁREA ACTIVOS - LIMIT 100
        ...
        // Combinar resultados y limitar total a 200
        $resultados = array_merge($articulos, $licencias);
        $resultados = array_slice($resultados, 0, 200);
